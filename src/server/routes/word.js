...
router.get("/", cache("20 minutes"), async (req, res) => {
    ...
    if(!req.query.keyword){ return res.status(400).send({ message: "parametro keyword não pode ser vazio." }); }
        ...
        let leoapi = await gettranslates(req.query.keyword).then((data) => {return data; });
        let def = await defineWord.define(req.query.keyword).then((data) => {return data; });
        //let trans = await translate(req.query.keyword, { from:'en', to: 'pt'}).then((data) => {return data; });
        let images = await bing.list({ keyword: req.query.keyword, num: 10, detail: true }).then((data) => {return data; });        
